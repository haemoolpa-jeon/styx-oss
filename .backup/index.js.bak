...
app.get('/health', (req, res) => {
  ...
  res.json({
    ...
    cache: {
      usersAge: usersCache ? Math.floor((now - usersCacheTime) / 1000) : null,
      sessionsAge: sessionsCache ? Math.floor((now - sessionsCacheTime) / 1000) : null,
      udpRateLimits: udpRateLimits?.size || 0
    },
    udp: typeof udpStats !== 'undefined' ? {
      clients: udpClients?.size || 0,
      rooms: roomMembers?.size || 0,
      ...udpStats
    } : null
  });
});
...
// 비동기 파일 작업 (with caching)
const loadUsers = async () => {
  const now = Date.now();
  if (usersCache && now - usersCacheTime < USERS_CACHE_TTL) {
    return JSON.parse(JSON.stringify(usersCache)); // Return copy to prevent mutation
  }
  try {
...
const saveUsers = async (data) => {
  try {
    ...
    usersCache = JSON.parse(JSON.stringify(data)); // Store copy
...
const loadSessions = async () => {
  ...
  if (sessionsCache && now - sessionsCacheTime < SESSIONS_CACHE_TTL) {
    return new Map(sessionsCache); // Return copy
  }
  try {
...
const saveSessions = async (sessions) => {
  try {
    ...
    sessionsCache = [...sessions.entries()]; // Update cache
...
setInterval(async () => {
  ...
  // Cleanup UDP rate limits
  for (const [ip, record] of udpRateLimits) {
    if (now - record.windowStart > UDP_RATE_WINDOW * 2) udpRateLimits.delete(ip);
  }
  // Cleanup suspicious IPs (remove after penalty expires + 1 hour)
...
io.on('connection', (socket) => {
  ...
  socket.on('admin-whitelist-add', ({ ip }, cb) => {
    if (!socket.isAdmin) return cb?.({ error: 'Not authorized' });
    // Validate IPv4 format with proper octet range (0-255)
...
// UDP rate limiting per IP
const UDP_RATE_LIMIT = 500; // packets per second per IP
const UDP_RATE_WINDOW = 1000; // 1 second window
const udpRateLimits = new Map(); // ip -> { count, windowStart }
