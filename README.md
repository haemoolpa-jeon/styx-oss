...
## 🔴 v1.5.3 서버 모듈화 및 보안 강화

### 📦 서버 코드 모듈화
1,730줄 단일 파일에서 10개 모듈로 분리:

```
server/
├── index.js           (268줄) - 진입점, HTTP 라우트
├── config.js          (63줄)  - 환경 설정
├── handlers/socket.js (530줄) - Socket.IO 핸들러
├── middleware/security.js (158줄) - 속도 제한, 화이트리스트
├── services/
│   ├── users.js       (182줄) - 사용자 CRUD + 캐싱
│   ├── sessions.js    (153줄) - 세션 관리
│   ├── rooms.js       (173줄) - 방 관리
│   └── udp.js         (214줄) - UDP 릴레이
└── utils/
    ├── audit.js       (34줄)  - 순환 버퍼 로깅
    └── validation.js  (71줄)  - 입력 검증
```

### 🛡️ 보안 수정
- **관리자 토큰 폴백 제거** - 프로덕션에서 ADMIN_TOKEN 필수
- **IP 검증 강화** - 0-255 범위 검증 정규식 수정
- **메모리 누수 방지** - suspiciousIPs, UDP 속도 제한 정리
- **캐시 변이 버그 수정** - 깊은 복사로 참조 문제 해결

### ⚡ 성능 개선
- **사용자 데이터 캐싱** - 5초 TTL
- **세션 캐싱** - 5초 TTL
- **UDP DoS 방지** - IP당 500 패킷/초 제한

### 🧪 테스트
- **19개 유닛 테스트** 추가 (`npm test`)
- config, validation, audit, security, rooms, UDP 모듈 커버

### 🦀 Rust 안전성
- Tauri 앱의 모든 `.unwrap()` 호출을 적절한 에러 처리로 교체

---

## 🔴 v1.5.2 코드 모듈화 및 품질 개선

### 📦 코드 모듈화
app.js에서 9개 모듈 추출로 유지보수성 향상:
- **utils.js** (55줄) - 품질 등급, 에러 메시지, 시간 포맷
- **toast.js** (23줄) - 토스트 알림
- **accessibility.js** (144줄) - 접근성 설정, 스크린 리더
- **recording.js** (208줄) - 멀티트랙/루프백/믹스다운 녹음
- **sync.js** (144줄) - 동기화 모드, 장치 지연 보정
- **sound.js** (37줄) - 입장/퇴장 알림음
- **theme.js** (35줄) - 다크/라이트 테마
- **tuner.js** (78줄) - 악기 튜너
- **keyboard.js** (301줄) - 키보드 단축키

**결과:** app.js 6,361줄 → 5,497줄 (-864줄, -13.6%)

### 🐛 버그 수정
- 소켓 재연결 시 핸들러 중복 등록 방지
- 모듈 변수 참조 오류 수정
- 메모리 누수 방지 (AudioContext cleanup)
- 키보드 단축키 자동 초기화
...
## 🔴 v1.5.1 버그 수정 및 오디오 개선

### 🐛 버그 수정
- 템플릿 드롭다운/BPM 스피너 다크/라이트 모드 스타일링
- 녹음 버튼 아이콘 전용으로 변경
- 웹 브라우저 입력 미터 활성화
- 웹 브라우저 피어 카드 표시 수정
- peer.pc.close() null safety 추가
- 진단 모달 데이터 수집 수정

### 🎵 오디오 개선
- Sync Mode 장치 지연 보정 (loopback 테스트)
- PLC fade-out 개선 (연속 패킷 손실 시)
- 지연 측정에 장치 지연 포함
