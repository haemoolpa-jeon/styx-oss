...
**Styx 강점**: 웹 브라우저로 즉시 사용 가능, 설치 불필요
**Styx 약점**: 지연 시간, 개인 믹서 기능 부족
...
**구현 방법**
- Web Audio API의 StereoPannerNode 사용
- 뮤트: gainNode.gain.value = 0
- 솔로: 다른 모든 피어 gainNode를 0으로

**구현 난이도**: 쉬움 (2-3시간)
**예상 비용**: 없음

**기대 효과**
- 여러 명이 동시에 말할 때 특정 사람만 집중해서 듣기 가능
- 스테레오 공간감으로 누가 말하는지 구분 용이
- 전문 음악 협업 도구 수준의 믹싱 경험

**예상 결과물**
- UI: 각 사용자 카드에 M/S 버튼, 팬 슬라이더 추가
- 기능: 실시간 개인 믹싱

---

### 2. 메트로놈 개선 (카운트인/강박 표시)
...
- **카운트인**: 시작 전 4박(또는 설정값) 카운트
- **박자 선택**: 4/4, 3/4, 6/8 등
- **강박 표시**: 첫 박은 높은 음, 나머지는 낮은 음
- **시각적 비트**: 현재 박자 위치 표시

**구현 방법**
- 카운트인: setTimeout으로 N박 후 실제 시작
- 강박: 주파수 다른 오실레이터 (예: 1박=880Hz, 나머지=440Hz)
- 시각적 비트: CSS 애니메이션으로 현재 박자 하이라이트

**구현 난이도**: 쉬움 (1-2시간)
**예상 비용**: 없음

**기대 효과**
- 합주 시작 시 모든 참가자가 동시에 시작 가능
- 강박 구분으로 박자 파악 용이
- 시각적 피드백으로 청각 장애인도 사용 가능

**예상 결과물**
- UI: 박자 선택 드롭다운, 카운트인 체크박스, 비트 인디케이터
- 오디오: 강박/약박 구분되는 클릭 소리
